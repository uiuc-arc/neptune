 ...
 * under the License.
 */
#ifndef TVM_INCL_TIR_SCHEDULE_UTILS_H_
#define TVM_INCL_TIR_SCHEDULE_UTILS_H_
...
struct BlockMap {
  //! \brief Create a block map from a statement by visiting it and adding all blocks to the map
  //! (each block mapping to itself initially).
  static BlockMap CreateFromStmt(const Stmt& stmt);

  //! \brief Given an entry `(b -> c)`, find what maps to `b` (call it `a`), and update the entry to
  //! `(a -> c)`. If `a` does not exist, then `b` is a new block, and we do nothing.
  void Insert(Block b, Block c);

  /*!
   \brief Prune the block map over a statement. Removes all entries that are not in the statement.
   \note Use this function carefully, as it will make ValidateOverStmt pass trivially. Explicitly
     not inserting blocks is preferred when possible.
  */
  void PruneOverStmt(const Stmt& stmt);

  //! \brief Validate the block map over a statement. Checks that for each `a -> b` in the map, `b`
  //! exists in the statement.
  void ValidateOverStmt(const Stmt& stmt);

  Map<Block, Block> GetFwdMap() const { return fwd_map_; }

 private:
  BlockMap() = default;

  Map<Block, Block> fwd_map_, bwd_map_;
};
...
#endif  // TVM_INCL_TIR_SCHEDULE_UTILS_H_
